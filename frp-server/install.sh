...
install_frps_binary() {
  ...
  tmp_dir="/tmp/frp-${FRP_VERSION}-server"

  run curl -fL "$url" -o "/tmp/${archive}"
  run rm -rf "$tmp_dir"
  run mkdir -p "$tmp_dir"
  run tar -xzf "/tmp/${archive}" -C "$tmp_dir" --strip-components=1
  run sudo install -m 755 "$tmp_dir/frps" "$FRP_BINARY_PATH"
}
...
if [[ "$DRY_RUN" == "1" || "$DRY_RUN" == "true" ]]; then
  print_cmd sudo tee /etc/systemd/system/frps.service
else
  sudo tee /etc/systemd/system/frps.service >/dev/null <<EOF
[Unit]
Description=frp server for private GitLab access
